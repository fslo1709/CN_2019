...
int main(int argc, char* argv[]){
    ...
    if(argc != 7){
        fprintf(stderr,"Format: %s <sender IP> <recv IP> <sender port> <agent port> <recv port> <loss_rate>\n", argv[0]);
        fprintf(stderr, "For example: ./agent local local 8887 8888 8889 0.3\n");
        exit(1);
    } else {
        setIP(ip[0], argv[1]);
        setIP(ip[1], local_str);
        setIP(ip[2], argv[2]);
    ...
    while(1){
        ...
        if(segment_size > 0){
            ...
            if(strcmp(ipfrom, ip[0]) == 0 && portfrom == port[0]) {
                ...
                if(s_tmp.head.ack == 0) {
                    fprintf(stderr, "Received receiver's non-ack segment\n");
                    exit(1);
                }
                if(s_tmp.head.fin == 1) {
